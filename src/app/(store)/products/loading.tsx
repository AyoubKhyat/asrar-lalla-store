  ...
  return (
    <div className="min-h-screen bg-bg animate-pulse" style={{ paddingTop: "var(--total-header)" }}>
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-12">
        <div className="h-10 w-48 bg-border/50 rounded-2xl mb-4" />
        <div className="h-5 w-72 bg-border/30 rounded-full mb-8" />
        <div className="flex gap-3 mb-8">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-9 w-20 bg-border/40 rounded-full" />
          ))}
        </div>
